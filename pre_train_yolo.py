...
parser.add_argument('--out', '-o', default='comp_random.model',
...
cropwidth = 500 - model.insize

def read_image(path, center=False, flip=False):
    # Data loading routine
    image = np.asarray(Image.open(path).convert("RGB").resize((500,500))).transpose(2, 0, 1)[::-1]
    if center:
...
def log_result():
    ...
    while True:
        ...
        if train:
            ...
            sys.stderr.write(
                '\rtrain {0} updates ({1} samples) time: {2} ({3:.4} images/sec) acc={4:.4} '
